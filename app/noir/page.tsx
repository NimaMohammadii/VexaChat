                ...
                return (
                  <button
                    ...
                    className={`flex h-12 w-full items-center justify-between rounded-xl px-3 text-left transition-all ${
                      isSelected ? "border border-[#FF2E63]/35 bg-white/[0.05] shadow-[0_0_14px_rgba(255,46,99,0.2)]" : "border border-transparent bg-transparent hover:bg-white/[0.05]"
                    }`}
                  >
  ...
  return (
    <RealtimeKitProvider value={meeting}>
      <div className="absolute inset-0 bg-black">
        {meeting ? <RtkMeeting meeting={meeting} mode="fill" showSetupScreen={false} /> : null}
        <button
          type="button"
          onClick={onLeave}
          className="absolute left-4 top-4 z-30 flex h-10 items-center justify-center rounded-full border border-white/10 bg-black/70 px-4 text-xs font-semibold uppercase tracking-[0.14em] text-white/80 backdrop-blur"
        >
          Leave
        </button>
      </div>
...
export default function NoirPage() {
  ...
  return (
    ...
    >
      <motion.div initial={{ opacity: 0.7, scale: 0.985 }} animate={{ opacity: started ? 1 : 0.62, scale: started ? 1 : 0.992 }} transition={{ duration: 0.4, ease: "easeOut" }} className="flex h-full min-h-0 flex-col pb-3">
        <header className="flex h-14 shrink-0 items-center justify-between px-4">
          <button type="button" aria-label="Back" onClick={() => void stopSession()} className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/[0.05] text-white backdrop-blur transition-all hover:border-[#FF2E63]/45 hover:shadow-[0_0_18px_rgba(255,46,99,0.22)]">
          ...
          <button type="button" aria-label="Settings" className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/[0.05] text-white/85 backdrop-blur transition-all hover:border-[#FF2E63]/45 hover:shadow-[0_0_18px_rgba(255,46,99,0.22)]">
        ...
        <footer className="shrink-0 px-4 pt-3">
          <div className="mx-auto flex max-w-md flex-wrap items-center justify-center gap-2">
            <PillButton variant="skip" onClick={() => void skipSession()}>
              <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5"><path d="M4 7l6 5-6 5V7Zm8 0 6 5-6 5V7Z" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" /></svg>
            </PillButton>
            <CircleButton>
              <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5"><path d="M12 4a2.5 2.5 0 0 1 2.5 2.5V12A2.5 2.5 0 1 1 9.5 12V6.5A2.5 2.5 0 0 1 12 4Z" stroke="currentColor" strokeWidth="1.6" /><path d="M7 11.5a5 5 0 1 0 10 0M12 17v3M9.5 20h5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" /></svg>
            </CircleButton>
            <CircleButton>
              <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5"><rect x="4" y="7" width="11" height="10" rx="2" stroke="currentColor" strokeWidth="1.6" /><path d="M15 10l5-2v8l-5-2" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" /></svg>
            </CircleButton>
            <CircleButton active={friendPending} onClick={() => setFriendPending((value) => !value)}>
              <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5">{friendPending ? <path d="M6 12.5 10.2 17 18 8.5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /> : <><circle cx="10" cy="9" r="3" stroke="currentColor" strokeWidth="1.6" /><path d="M4.5 18a5.5 5.5 0 0 1 11 0M18.5 8v6M15.5 11h6" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" /></>}</svg>
            </CircleButton>
            <PillButton variant="stop" onClick={() => void stopSession()}>
              <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5"><rect x="7" y="7" width="10" height="10" rx="1.8" stroke="currentColor" strokeWidth="1.8" /></svg>
            </PillButton>
          </div>
        </footer>
      </motion.div>
      ...
      <AnimatePresence>
        {!started ? (
          <motion.div key="overlay" initial={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.965 }} transition={{ duration: 0.4, ease: [0.32, 0.72, 0, 1] }} className="absolute inset-0 z-20 flex items-center justify-center bg-black px-7">
            <div className="w-full max-w-sm text-center">
              ...
              <button type="button" onClick={() => void startSession()} disabled={searching} className="mt-8 h-[52px] w-full rounded-full border border-[#FF2E63]/45 bg-[#FF2E63]/10 px-8 text-sm font-medium tracking-[0.08em] text-white shadow-[0_0_30px_rgba(255,46,99,0.25)] transition-all duration-200 hover:bg-[#FF2E63]/18 active:scale-[0.98] disabled:opacity-60">
